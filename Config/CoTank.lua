...
local function CreateCoTankPanel()
    ...
    makeCheckbox(child, nextY(), "unlock", L["COMMON_UNLOCK"])
    ...
    local xLabel = child:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    xLabel:SetPoint("TOPLEFT", PAD_LEFT, nextY())
    xLabel:SetText((L["COTANK_OFFSET_X"] or "X offset") .. ": " .. (db.x or 200))
    local xSlider = CreateFrame("Slider", "TankStuff_CoTankX", child, "OptionsSliderTemplate")
    xSlider:SetPoint("TOPLEFT", PAD_LEFT, y - 20)
    xSlider:SetMinMaxValues(-400, 400)
    xSlider:SetValueStep(1)
    xSlider:SetValue(db.x or 200)
    xSlider:SetScript("OnValueChanged", function(self, val)
        db.x = math.floor(val)
        xLabel:SetText((L["COTANK_OFFSET_X"] or "X offset") .. ": " .. db.x)
        RefreshDisplay()
    end)
    setSliderLabels(xSlider, "-400", "400")
    nextY()
    nextY()

    local yLabel = child:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    yLabel:SetPoint("TOPLEFT", PAD_LEFT, nextY())
    yLabel:SetText((L["COTANK_OFFSET_Y"] or "Y offset") .. ": " .. (db.y or 0))
    local ySlider = CreateFrame("Slider", "TankStuff_CoTankY", child, "OptionsSliderTemplate")
    ySlider:SetPoint("TOPLEFT", PAD_LEFT, y - 20)
    ySlider:SetMinMaxValues(-400, 400)
    ySlider:SetValueStep(1)
    ySlider:SetValue(db.y or 0)
    ySlider:SetScript("OnValueChanged", function(self, val)
        db.y = math.floor(val)
        yLabel:SetText((L["COTANK_OFFSET_Y"] or "Y offset") .. ": " .. db.y)
        RefreshDisplay()
    end)
    setSliderLabels(ySlider, "-400", "400")
    ...
    makeCheckbox(child, nextY(), "nameColorUseClassColor", L["COTANK_NAME_USE_CLASS_COLOR"])
